...
// 新增：要素选择相关变量
const selectedFeatures = ref(new Set()) // 存储被选择的要素
const featureHighlightLayers = ref(new Map()) // 存储高亮图层
...
const activateSelectFeature = () => {
  ...
  // 显示提示信息
  showZoomMessage('👆 请点击地图上的要素进行选择，再次点击可取消选择', 'info')
}
...
const handleKeyDown = (e) => {
  ...
  // Delete键清除所有要素选择
  if (e.key === 'Delete' && selectedFeatures.value.size > 0) {
    e.preventDefault()
    clearAllFeatureSelections()
    showZoomMessage('🗑️ 已清除所有要素选择', 'info')
  }
}
...
const handleFeatureSelection = (e) => {
  ...
  console.log('开始处理要素选择，点击位置:', e.latlng)
  ...
  if (clickedFeature) {
    ...
    console.log('生成的要素ID:', featureId)
    
    if (selectedFeatures.value.has(featureId)) {
      // 如果要素已经被选择，则取消选择
      console.log('要素已被选择，执行取消选择')
      deselectFeature(featureId, clickedFeature)
      showZoomMessage('✅ 已取消选择要素', 'success')
    } else {
      // 如果要素未被选择，则选择它
      console.log('要素未被选择，执行选择')
      selectFeature(featureId, clickedFeature)
      showZoomMessage('✅ 已选择要素', 'success')
    }
  } else {
    // 如果点击空白区域，清除所有选择
    console.log('未找到要素，点击空白区域')
...
// 查找点击位置下的要素
const findFeatureAtPoint = (latlng) => {
  if (!map.value) return null
  
  console.log('开始查找要素，点击位置:', latlng)
  console.log('当前图层状态:', layers)
  console.log('图层对象:', layerObjects)
  
  // 遍历所有可见图层
  for (const [layerType, layer] of Object.entries(layerObjects)) {
    if (!layers[layerType] || !layer) {
      console.log(`跳过${layerType}图层:`, { visible: layers[layerType], layer: !!layer })
      continue
    }
    
    console.log(`检查${layerType}图层:`, layer)
    
    try {
      if (layerType === 'monitoringPoints' || layerType === 'settlements') {
        // 点要素
        console.log(`在${layerType}图层中查找点要素`)
        const foundFeature = findPointFeatureAtLocation(layer, latlng)
        if (foundFeature) {
          console.log(`在${layerType}图层中找到点要素:`, foundFeature)
          return { type: layerType, feature: foundFeature, layer }
        }
      } else if (layerType === 'waterLines') {
        // 线要素
        console.log(`在${layerType}图层中查找线要素`)
        const foundFeature = findLineFeatureAtLocation(layer, latlng)
        if (foundFeature) {
          console.log(`在${layerType}图层中找到线要素:`, foundFeature)
          return { type: layerType, feature: foundFeature, layer }
        }
      } else {
        // 面要素
        console.log(`在${layerType}图层中查找面要素`)
        const foundFeature = findPolygonFeatureAtLocation(layer, latlng)
        if (foundFeature) {
          console.log(`在${layerType}图层中找到面要素:`, foundFeature)
          return { type: layerType, feature: foundFeature, layer }
        }
      }
    } catch (error) {
      console.error(`查找${layerType}图层要素时出错:`, error)
    }
  }
...
// 查找点要素
const findPointFeatureAtLocation = (layer, latlng) => {
  let foundFeature = null
  let minDistance = Infinity
  
  console.log('开始查找点要素，图层:', layer)
  console.log('点击位置:', latlng)
  
  layer.eachLayer((sublayer) => {
    if (sublayer.getLatLng) {
      const sublayerLatlng = sublayer.getLatLng()
      const distance = latlng.distanceTo(sublayerLatlng)
      console.log('检查子图层:', { sublayer, latlng: sublayerLatlng, distance })
      
      if (distance < 20 && distance < minDistance) { // 20像素容差
        minDistance = distance
        foundFeature = sublayer.feature
        console.log('找到更近的点要素:', { foundFeature, distance })
      }
    } else {
      console.log('子图层没有getLatLng方法:', sublayer)
    }
  })
  
  console.log('点要素查找结果:', { foundFeature, minDistance })
  return foundFeature
}

// 查找面要素
const findPolygonFeatureAtLocation = (layer, latlng) => {
  let foundFeature = null
...
// 查找线要素
const findLineFeatureAtLocation = (layer, latlng) => {
  let foundFeature = null
  let minDistance = Infinity
  
  layer.eachLayer((sublayer) => {
    if (sublayer.getLatLngs) {
      const distance = getDistanceToLine(latlng, sublayer.getLatLngs())
      if (distance < 15 && distance < minDistance) { // 15像素容差
        minDistance = distance
        foundFeature = sublayer.feature
      }
    }
...
// 计算点到线的距离
const getDistanceToLine = (point, linePoints) => {
  let minDistance = Infinity
  
  for (let i = 0; i < linePoints.length - 1; i++) {
    const distance = pointToLineDistance(point, linePoints[i], linePoints[i + 1])
...
const pointToLineDistance = (point, lineStart, lineEnd) => {
  ...
  if (param < 0) return point.distanceTo(lineStart)
  if (param > 1) return point.distanceTo(lineEnd)
  
  const x = lineStart.lat + param * C
  const y = lineStart.lng + param * D
  
  return point.distanceTo(L.latLng(x, y))
}

// 生成要素唯一ID
const getFeatureId = (clickedFeature) => {
  ...
  if (feature.properties && feature.properties.id) {
    return `${feature.properties.id}_${layerType}`
  }
  if (feature.properties && feature.properties.NAME) {
    return `${feature.properties.NAME}_${layerType}`
  }
  if (feature.properties && feature.properties.name) {
    return `${feature.properties.name}_${layerType}`
  }
  return `${layerType}_${Date.now()}_${Math.random()}`
}

// 选择要素
const selectFeature = (featureId, clickedFeature) => {
  selectedFeatures.value.add(featureId)
  
  // 创建高亮图层
  const highlightLayer = createFeatureHighlight(clickedFeature)
  if (highlightLayer) {
    featureHighlightLayers.value.set(featureId, highlightLayer)
    highlightLayer.addTo(map.value)
  }
  
  console.log('要素已选择:', featureId, clickedFeature)
}

// 取消选择要素
const deselectFeature = (featureId, clickedFeature) => {
  selectedFeatures.value.delete(featureId)
  
  // 移除高亮图层
  const highlightLayer = featureHighlightLayers.value.get(featureId)
  if (highlightLayer) {
    map.value.removeLayer(highlightLayer)
    featureHighlightLayers.value.delete(featureId)
  }
