...
export default {
  name: 'LakePollutionSimulationPrediction',
  props: {
    map: {
      type: Object,
      required: true
  ...
  setup(props, { emit }) {
    ...
    // 修改湖泊选项初始化，适配Beijing-GeoJson/目录下的数据
    const initializeLakeOptions = () => {
      if (props['water-areas-layer']) {
        // 这里假设传入的water-areas-layer就是Beijing-GeoJson/北京市_水系面数据.json的图层
        const features = props['water-areas-layer'].toGeoJSON().features
        lakeOptions.value = features.map((feature, index) => ({
          id: feature.properties.osm_id || feature.properties.OSM_ID || feature.properties.id || index + 1,
    ...
    // 在onMounted时加载各类GeoJSON数据并创建图层对象，类似主页面实现
    // 这里需要根据实际的GeoJSON文件路径和图层名称来加载
    // 例如：
    // const loadGeoJSON = async (url, layerName) => {
    //   const response = await fetch(url);
    //   const geojson = await response.json();
    //   layerObjects[layerName] = L.geoJSON(geojson);
    //   if (layers[layerName]) {
    //     layerObjects[layerName].addTo(props.map);
    //   }
    // };
    // await loadGeoJSON('path/to/districts.geojson', 'districts');
    // await loadGeoJSON('path/to/waterLines.geojson', 'waterLines');
    // await loadGeoJSON('path/to/waterAreas.geojson', 'waterAreas');
    // await loadGeoJSON('path/to/reservoirs.geojson', 'reservoirs');
    // await loadGeoJSON('path/to/settlements.geojson', 'settlements');
